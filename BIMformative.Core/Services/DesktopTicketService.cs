...
namespace BIMformative.Core.Services
{
    public class DesktopTicketService : IDesktopTicketService
    {
        ...
        public async Task<string> CreateTicketAsync(CancellationToken ct)
        {
            if (!_authService.IsAuthenticated || string.IsNullOrEmpty(_authService.AccessToken))
                throw new InvalidOperationException("User is not authenticated.");

            var request = new HttpRequestMessage(HttpMethod.Post, TicketEndpoint);
            ...
            try
            {
                var response = await _publicHttp.SendAsync(request, ct);
                ...
                var result = JsonConvert.DeserializeObject<TicketResponse>(json);
